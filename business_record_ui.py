...
def _widget(field: dict[str, Any], value: Any, key: str, disabled: bool = False):
    ...
    if typ == "datetime":
        return st.text_input(label, value=str(value or ""), key=key, help=help_text or "格式：年-月-日 时:分", disabled=disabled)
    if typ == "select":
...
def render_parameters(kind: str, record: dict[str, Any], key_prefix: str, editable_labels: set[str] | None = None) -> tuple[dict[str, Any], str]:
    ...
    # 粗糙度：实时显示三组实测值的算术平均
    # 粗糙度：三个实测值后面紧跟实时平均值
    repeat_keys = {"repeat_check_1", "repeat_check_2", "repeat_check_3"}
    repeat_fields = [f for f in other_manual if f["key"] in repeat_keys]
    core_fields = [f for f in other_manual if f["key"] not in repeat_keys]
    if other_manual:
        with st.expander("本次核查与实际记录", expanded=True):
            st.caption("填写仪器核查、过程实测和本次特有信息")
            cols = st.columns(3)
            # 先渲染普通字段
            ...
            if repeat_fields:
                ...
                # 实时计算并显示平均值
                try:
                    r1 = float(params.get("repeat_check_1", 0) or 0)
                    r2 = float(params.get("repeat_check_2", 0) or 0)
                    r3 = float(params.get("repeat_check_3", 0) or 0)
                    if r1 and r2 and r3:
                        avg = round((r1 + r2 + r3) / 3, 4)
                    else:
                        avg = None
                except (ValueError, TypeError):
                    avg = None
                with rcols[3]:
                    st.metric("实测平均值/μm", f"{avg}" if avg is not None else "—")
                if avg is not None:
                    params["standard_block_measured"] = avg
