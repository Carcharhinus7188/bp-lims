...
SCHEMAS = {
    "rough": {
        ...
        "sections": [
            ...
            {"title": "试验参数与使用前确认", "fields": [
                ...
                {"key": "sampling_length", "label": "取样长度/mm", "type": "number", "default": 2.5},
                {"key": "sampling_count", "label": "取样个数", "type": "number", "default": 3.0},
                {"key": "evaluation_length", "label": "评定长度/mm", "type": "number", "default": 7.5},
                ...
                {"key": "three_length_mode", "label": "评定长度方式", "type": "select", "options": ["3L（已完成方法确认）", "5L（默认）", "自定义"], "default": "3L（已完成方法确认）"},
            ]},
        ],
        "columns": [
    ...
    "thickness": {
        ...
        "sections": [
            ...
            {"title": "影像测量参数", "fields": [
                ...
                {"key": "design_thickness", "label": "设计厚度/mm", "type": "number", "default": 1.0, "actual": True},
            ]},
        ],
        "columns": [
            ("sample_no", "试样编号", "text"),
            ("r1_fixed", "第1次·固定端/mm", "number"), ("r1_middle", "第1次·中点/mm", "number"), ("r1_free", "第1次·自由端/mm", "number"),
            ("r2_fixed", "第2次·固定端/mm", "number"), ("r2_middle", "第2次·中点/mm", "number"), ("r2_free", "第2次·自由端/mm", "number"),
            ("r3_fixed", "第3次·固定端/mm", "number"), ("r3_middle", "第3次·中点/mm", "number"), ("r3_free", "第3次·自由端/mm", "number"),
    ...
    "fixed_denture": {
        ...
        "sections": [
            {"title": "环境与设备", "fields": COMMON_ENV_FIELDS + COMMON_DEVICE_FIELDS},
            {"title": "资料、模型与追溯性核查", "fields": [
                {"key": "preset_model_check", "label": "预设模型及对颌模型核查", "type": "select", "options": ["符合", "不符合", "不适用"], "default": "符合", "actual": True},
                {"key": "design_sheet_check", "label": "设计单完整性与一致性", "type": "select", "options": ["符合", "不符合"], "default": "符合", "actual": True},
                {"key": "material_traceability", "label": "主要原材料注册证及批号追溯", "type": "select", "options": ["符合", "不符合", "不适用"], "default": "符合", "actual": True},
                {"key": "production_record_check", "label": "制作过程记录完整性", "type": "select", "options": ["完整", "不完整", "不适用"], "default": "完整", "actual": True},
                {"key": "design_requirement", "label": "委托/产品技术要求及特殊设计说明", "type": "text", "default": "按委托资料及YY/T 1936-2024执行", "actual": True},
            ]},
        ],
        "columns": [
            ("sample_no", "样品编号", "text"),
            ("surface_property", "表面性能", "select:符合|不符合|不适用"),
            ("tissue_surface", "组织面", "select:符合|不符合|不适用"),
            ("fit", "适合性", "select:符合|不符合|不适用"),
            ("pontic", "桥体", "select:符合|不符合|不适用"),
            ("interface_thickness", "交界线厚度/mm", "number"),
            ("base_thickness", "基底厚度/mm", "number"),
            ("connector_location", "连接体部位", "text"),
            ("connector_full_grids", "连接体完全包含网格数n", "number"),
            ("connector_traced_grids", "连接体含描记线网格数m", "number"),
            ("connector_area", "连接体面积s/mm²", "calc"),
            ("outline", "外形轮廓", "select:符合|不符合|不适用"),
            ("adjacency", "邻接关系", "select:符合|不符合|不适用"),
            ("occlusion", "咬合关系", "select:符合|不符合|不适用"),
            ("color", "颜色", "select:符合|不符合|不适用"),
            ("interface_line", "交界线", "select:符合|不符合|不适用"),
            ("metal_exposed", "金属是否暴露", "select:否|是|不适用"),
            ("roughness_visual", "粗糙度视觉比较", "select:符合|不符合|不适用"),
            ("roughness_tactile", "粗糙度触觉比较", "select:符合|不符合|不适用"),
            ("pore_gt_30", ">30μm孔隙数", "number"),
            ("pore_40_150", "40-150μm孔隙数", "number"),
            ("pore_gt_150", ">150μm孔隙", "select:无|有|不适用"),
            ("image_no", "显微/测量图像编号", "text"),
            ("conclusion", "单样综合结论", "calc"),
            ("note", "备注", "text"),
        ],
    },
    "removable_denture": {
        ...
        "sections": [
            {"title": "环境与设备", "fields": COMMON_ENV_FIELDS + COMMON_DEVICE_FIELDS},
            {"title": "资料、模型与综合试验条件", "fields": [
                {"key": "material_traceability", "label": "主要原材料及批号追溯", "type": "select", "options": ["符合", "不符合", "不适用"], "default": "符合", "actual": True},
                {"key": "design_sheet_check", "label": "设计单、模型与附件一致性", "type": "select", "options": ["符合", "不符合"], "default": "符合", "actual": True},
                {"key": "production_record_check", "label": "制作过程记录完整性", "type": "select", "options": ["符合", "不符合"], "default": "符合", "actual": True},
                {"key": "d65_illuminance", "label": "色泽检查照度/lx", "type": "number", "default": 700.0, "actual": True},
            ]},
        ],
        "columns": [
            ("sample_no", "样品编号", "text"),
            ("sample_type", "义齿类型", "select:全口义齿|可摘局部义齿|金属支架可摘局部义齿"),
            ("exterior_surface", "外形及表面", "select:合格|不合格"),
            ("root_embedding", "牙根外形/金属支架包埋", "select:合格|不合格|不适用"),
            ("fit_base", "适合性-基托/支架", "select:合格|不合格"),
            ("fit_clasp", "适合性-卡环", "select:合格|不合格|不适用"),
            ("fit_rest", "适合性-支托", "select:合格|不合格|不适用"),
            ("color", "色泽", "select:合格|不合格"),
            ("tooth_arrangement", "人工牙排列及牙合曲线", "select:合格|不合格|不适用"),
            ("occlusion", "咬合", "select:合格|不合格"),
            ("marking_docs", "标志和随附文件", "select:合格|不合格"),
            ("packaging", "包装", "select:合格|不合格"),
            ("edge_1", "基托边缘测量1/mm", "number"),
            ("edge_2", "基托边缘测量2/mm", "number"),
            ("edge_3", "基托边缘测量3/mm", "number"),
            ("edge_mean", "基托边缘平均值/mm", "calc"),
            ("middle_1", "基托中部测量1/mm", "number"),
            ("middle_2", "基托中部测量2/mm", "number"),
            ("middle_3", "基托中部测量3/mm", "number"),
            ("middle_mean", "基托中部平均值/mm", "calc"),
            ("thickness_conclusion", "厚度判定", "calc"),
            ("tissue_stop", "组织面终止点", "select:合格|不合格|不适用"),
            ("major_connector", "大连接体", "select:合格|不合格|不适用"),
            ("xray_quality", "金属内部质量", "select:合格|不合格|需复检|不适用"),
            ("finish_line", "内、外终止线", "select:合格|不合格|不适用"),
            ("porosity", "孔隙度", "select:合格|不合格|不适用"),
            ("image_no", "测量/显微/X射线图像编号", "text"),
            ("conclusion", "单样综合结论", "calc"),
            ("note", "备注", "text"),
        ],
    },
...
SUPPLEMENTAL_PROCESS_FIELDS = {
    ...
    "thickness": [
        ...
        {"key": "fixture_method_note", "label": "固定方式、测点布置及重复测量说明", "type": "text",
         "default": "固定端/中点/自由端各测1点并重复3次", "actual": True},
    ],
    "color": [
